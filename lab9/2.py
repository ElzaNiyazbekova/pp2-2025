...
# Создание NFA, который принимает строки с хотя бы тремя вхождениями "010"
nfa = graphviz.Digraph(format='png')
nfa.attr(rankdir='LR', size='8')
...
# Обозначаем начальное состояние
nfa.attr('node', shape='point')
nfa.node('start')
nfa.attr('node', shape='circle')
nfa.edge('start', initial_state)

# Обозначаем состояния
for state in states:
...
# Переходы для первого "010"
nfa.edge('q0', 'q1', label='0')
nfa.edge('q1', 'q2', label='1')
nfa.edge('q2', 'q3', label='0')

# Переходы для второго "010"
nfa.edge('q3', 'q4', label='0')
nfa.edge('q4', 'q5', label='1')
nfa.edge('q5', 'q6', label='0')

# Переходы для третьего "010"
nfa.edge('q6', 'q7', label='0')
nfa.edge('q7', 'q8', label='1')
nfa.edge('q8', 'q9', label='0')

# Возвратные переходы (по другим символам — для недетерминизма)
# Добавим возвратные петли на каждом состоянии по 0 и 1 (моделируем произвольные врезки)
for state in states:
    if state != accepting_state:
        nfa.edge(state, state, label='0,1')

nfa.render('/mnt/data/nfa_3x010', view=False)
'/mnt/data/nfa_3x010.png'
